...
def get_perms_data(config: Config, target_module_name: Optional[str] = None,
                     target_path: Optional[str] = None) -> List[Dict]:
    """
...
def execute_perms(config: Config, target_module_name: Optional[str] = None,
                  target_path: Optional[str] = None) -> List[Dict]:
    """
...
def display_perms_info(perms_list: List[Dict]) -> None:
    ...
    for entry in perms_list:
        ...
        if source:
            _log.info(f"  (src) 类型: {source['type']}, 权限: {source['mode']}, 所有者: {source['uid']}:{source['gid']}")
        else:
            _log.warn(f"源文件: {path} (不存在)")

        meta = entry['meta']
        _log.info(f"  (bak) 类型: {meta.get('type', 'unknown')}, 权限: {meta.get('mode', 'unknown')}, 所有者: {meta.get('uid', 'unknown')}:{meta.get('gid', 'unknown')}")
