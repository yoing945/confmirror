...
def _get_file_diff_lines(source: Path, backup: Path, context_lines: int = 3) -> Optional[List[str]]:
...
def compare_files_core(source: Path, backup: Path) -> FileDiffResult:
    ...
    if is_dir:
        return FileDiffResult(
            ...
            content_same=True, meta_same=meta_same,
            source_size=source_stat.st_size, backup_size=backup_stat.st_size,
            source_mtime=source_stat.st_mtime, backup_mtime=backup_stat.st_mtime,
            source_mode=oct(source_stat.st_mode)[-3:],
            backup_mode=meta_data.get('mode', 'N/A') if meta_data else 'N/A',
            source_uid=source_stat.st_uid, backup_uid=int(meta_data.get('uid', -1)) if meta_data else -1,
            source_gid=source_stat.st_gid, backup_gid=int(meta_data.get('gid', -1)) if meta_data else -1,
        )

    content_same = compare_content(source, backup)
    return FileDiffResult(
        source=source, backup=backup,
        content_same=content_same, meta_same=meta_same,
        source_size=source_stat.st_size, backup_size=backup_stat.st_size,
        source_mtime=source_stat.st_mtime, backup_mtime=backup_stat.st_mtime,
        source_mode=oct(source_stat.st_mode)[-3:],
        backup_mode=meta_data.get('mode', 'N/A') if meta_data else 'N/A',
...
def _collect_files_for_paths(config: Config, target_paths: List[str]) -> tuple[set, set]:
    """为指定路径收集源文件集合和备份文件集合。"""
...
def diff_module_core(config: Config, module_name: str, detail: bool = False) -> DiffResult:
    """对比整个模块，返回结构化结果。"""
    settings = config.settings
    backup_root = settings.backup_root
...
def diff_paths_core(config: Config, target_paths: List[str], detail: bool = False) -> DiffResult:
    """对比指定路径，返回结构化结果。"""
    settings = config.settings
    backup_root = settings.backup_root
