...
from confmirror.perms import get_perms_for_module, get_perms_for_path, matched_paths_to_perms_info
...
class TestGetPermsForModule:
    def test_script_module_returns_empty(self, tmp_path, caplog):
        ...
        config = Config(settings=settings, modules=[
            ModuleConfig(name="ufw", hook="ufw/script.sh")
        ])
    ...
    def test_path_module_returns_perms(self, tmp_path):
        ...
        with patch("confmirror.perms.glob.glob", return_value=[str(src / "sshd_config")]):
...
class TestExecuteList:
    def test_empty_modules(self, caplog):
        config = Config(settings=Settings(name="test", backup_root=Path("/tmp")), modules=[])
